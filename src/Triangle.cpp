...
Triangle::Triangle() : Triangle(Point(), Vector(), Vector())
{
}

Triangle::Triangle(const Point& vertex, const Vector& u, const Vector& v,
...
Vector Triangle::normal(const Vector& incident, const Point& point) const
{
...
double Triangle::intersect(const Ray& incident) const
{
    double a = u_.x();
    double b = v_.x();
    ...
    if (fabs(detA) > epsilon) {
        ...
        if ((fabs(s1)<epsilon || epsilon<s1) && (fabs(s1-1)<epsilon || (s1+epsilon)<1) &&
            ...
            (fabs(s1+s2)<epsilon || epsilon<s1+s2) && (fabs(s1+s2-1)<epsilon || (s1+s2+epsilon)<1))
            return t;
    }
    return -1;
}

Color Triangle::ambient() const
{
    return Model::ambient();
}

Color Triangle::diffuse(const Ray& incident, const Point& point, const Point& light) const
{
...
Color Triangle::specular(const Ray& incident, const Point& point, const Point& light) const
{
    Vector L = (light-point).normalized();
    Vector N = normal(incident.dir(), point);
...
Color Triangle::calc_phonglighting(const Ray& incident, const Point& point, const Point& light) const
{
